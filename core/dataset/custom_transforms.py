...
class RandomScaleCrop(object):
    ...
    def __call__(self, images, depths, nmaps, intrinsics):
        ...
        scaled_images = [cv2.resize(im, dsize=(scaled_w, scaled_h), interpolation=cv2.INTER_LINEAR).astype(np.float32) for im in images]
        ...
        scaled_nmaps = [cv2.resize(nm, dsize=(scaled_w, scaled_h), interpolation=cv2.INTER_NEAREST).astype(np.float32) for nm in nmaps]
        ...
        scaled_depths = [cv2.resize(dp, dsize=(scaled_w, scaled_h), interpolation=cv2.INTER_NEAREST).astype(np.float32) for dp in depths]
