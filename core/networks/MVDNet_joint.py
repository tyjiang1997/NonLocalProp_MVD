...
class MVDNet_joint(nn.Module):
    def __init__(self, cfg):
        super(MVDNet_joint, self).__init__()
        self.cfg = cfg
    ...
    def refine(self, depth, normal, img, dconf, nconf, K, iter=5, profiler=None):
        ...
        for i in range(iter):
            refined_depth, refined_normal = solver(refined_depth, refined_normal, img, dconf, nconf, K, profiler=profiler)
        return refined_depth, refined_normal

    def forward(self, target, refs, pose, tgt_depth, tgt_normal, intrinsics, intrinsics_inv, factor = None):
        profiler = Profiler()
        ...
        joint_depth_conf = torch.clamp(joint_depth_conf, 0.01, 1.0)
        ...
        refined_depth, refined_normal = self.refine(depth1, nmap_out, target, gt_depth_conf, gt_normal_conf, intrinsics, iter=self.iter, profiler=None)
